...
def deploy(path_to_ssh_key_private_key, server_address, prefix):   
    ...
    def connect():
        ...
        stdin, stdout, stderr = ssh_client.exec_command("screen -dm python /home/testtest/Sprint1/Sprint2_v2/server.py %s -c 'sleep 30; exec sh'" %(prefix))
        print("Server launched, receiving request...")
        
        time.sleep(2)
        
        stdin, stdout, stderr = ssh_client.exec_command('(crontab -l; echo "*/2 * * * * python /home/testtest/Sprint1/Sprint2_v2/rawrotator.py %s") | crontab -' %prefix)
        
        time.sleep(5)
        
        stdin, stdout, stderr = ssh_client.exec_command('(crontab -l; echo "*/2 * * * * python /home/testtest/Sprint1/Sprint2_v2/procrotator.py %s") | crontab -' %prefix)
